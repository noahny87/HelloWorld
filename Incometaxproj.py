...
def tax_func(state, income):
    ...
    if 'State ' in state_columns : 
        if state in df['State '].values:
             index_state = df[df['State '] == state].iloc[0]  # Get the index of the state entered

           # Get the tax rates for the state by using state index
             low_tax_rate = index_state ['Low Tax(%)']
             high_tax_rate = index_state ['High Tax(%)']
            # if income is in lower tax bracket
             if income <= 175781.23:
                ...
                #if income is in higher tax bracket
